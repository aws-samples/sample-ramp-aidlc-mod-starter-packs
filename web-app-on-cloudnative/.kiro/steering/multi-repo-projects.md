...
description: Repo-model decision gate (monorepo → monorepo-with-workspaces → domain-grouped repos → polyrepo) and the contract-first multi-repo flow — capture SYSTEM-LEVEL requirements, then system high-level design (contracts + topology + Wave 0), then SPLIT into derived per-repo specs and fan out. Use when the project spans multiple repositories, when deciding how to organize repos, when splitting/decomposing specs across SPA/BFF/engine repos, or for a monorepo spanning multiple contract-sharing domains.
---
