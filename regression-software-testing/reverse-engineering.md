...
## Step 4: Generate API Documentation (Contract-Testing Ready)

This step produces detailed API documentation that directly feeds API
contract testing. Every endpoint must be documented with enough detail
to generate contract tests that validate request/response structure,
status codes, and error handling survive an upgrade.
...
- **Path**: [/api/path] (include path variables with types)
- **Purpose**: [What it does]
- **Controller Class**: [Fully qualified class name]
- **Handler Method**: [Method name and signature]
- **Path Variables**: [List with names and types]
- **Query Parameters**: [List with names, types, required/optional]
- **Request Body**: [JSON structure with field types, or "none"]
- **Response Body**: [JSON structure with field types]
- **Success Status**: [HTTP status code, e.g., 200 OK]
- **Error Responses**:
  - [Status code] - [Condition] - [Exception class if applicable]
- **Dependencies**: [Factory/Model classes invoked]
- **Side Effects**: [DynamoDB writes, SNS notifications, etc.]

## API Contract Summary Table

| Method | Path | Request Body | Response Body | Success | Error Codes |
|--------|------|-------------|---------------|---------|-------------|
| [method] | [path] | [type or none] | [type] | [code] | [codes] |

## Internal Service Interfaces

### [Factory/Model Class Name]
- **Methods**: [List with full signatures]
- **Parameters**: [Parameter names, types, descriptions]
- **Return Types**: [Return type descriptions]
- **Exceptions Thrown**: [Exception classes and conditions]
- **Dependencies**: [Other classes/services called]

## Data Models (Request/Response Contracts)

### [Model Name]
- **DynamoDB Table**: [Table name]
- **Fields**:
  - [field name] - [type] - [DynamoDB attribute name] - [required/optional]
- **Relationships**: [Related models and how they connect]
- **Serialization Notes**: [Jackson annotations, custom serializers, etc.]
- **Validation Rules**: [Any constraints on field values]
```
