...
**Do not use long-lived static IAM user access keys** (`AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`). They are the leading source of leaked-credential incidents and secret-scanner alerts. Use short-lived, role-based credentials instead:

- **Local dev:** AWS IAM Identity Center — `aws sso login --profile <profile>`, then `export AWS_PROFILE=<profile>`. Terraform picks up the profile; no keys on disk.
- **EC2 / ECS / Lambda:** the instance profile / task role / execution role — no credentials configured at all.
- **CI/CD:** OIDC federation to an IAM role (GitHub Actions `aws-actions/configure-aws-credentials` with `role-to-assume`, or GitLab OIDC) — issues temporary STS credentials per job, nothing stored.
- **Bridging static keys:** if you are stuck with static keys, keep them in the OS keychain via `aws-vault` — never in plaintext env files or shell profiles.

If a Terraform input is itself a secret, pass it as a `TF_VAR_` **sourced at runtime** from a secret manager or a masked CI variable — never a literal in a script, shell history, or committed file:

```bash
# Value injected from the secret manager at runtime — never a literal here
...
Better still, avoid passing the secret through Terraform at all — see the RDS `manage_master_user_password` pattern above.
...
> **Never paste real *or example* access keys into code, scripts, or docs.** Even AWS's well-known documentation example keys (the `AKIA…EXAMPLE` pair) match secret-scanner signatures — GitGuardian, gitleaks, TruffleHog, and GitHub secret scanning flag them on sight and will trip enterprise security tooling. Use `<placeholders>` and runtime injection only.
...
- proposes long-lived static IAM user keys (`AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`) instead of short-lived role-based credentials (IAM Identity Center / SSO, instance/task roles, CI OIDC federation)
- pastes real or example access keys (e.g. the `AKIA…EXAMPLE` pair) into code/scripts/docs — these match secret-scanner signatures and trip enterprise security tooling
