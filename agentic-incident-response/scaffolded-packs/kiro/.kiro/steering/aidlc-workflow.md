...
- [ ] Phase 3: Tasks
  ...
  - [ ] 3d. Task execution (by wave)

## Current Status
**Phase**: [current phase]
**Status**: [In Progress / Awaiting Approval / Complete]
**Active Wave**: [wave number, during execution]
...
- **Network Exposure:** Public (internet-facing) vs internal/VPC-private vs partner/cross-account. Decide this here — it constrains API type, ingress, and testability downstream, and is expensive to retrofit. If "internal", "private", "not internet-facing", or "VPC-only" appears, capture it as a hard requirement now.
...
- **API & Communication:** API style, sync vs async, eventing pattern, auth strategy. Carry the Phase 1 **network-exposure** decision through here: internal-only/VPC-private ingress constrains the API type and requires private endpoints + VPC wiring — and means live end-to-end tests must run from inside the network boundary, not a developer laptop. Don't conflate "authenticated" with "not internet-facing".
...
**Focus:** Ordered, executable plan to build and verify the spec, organized into
**parallel groups** and dependency-ordered **waves**.
...
- **Definition of Done:** Tests pass + IaC deployed + smoke verified, or stricter. When the DoD includes deployment, require a **live smoke test** through the deployed path (fakes/mocks alone don't prove provider-side validation, invoke-time permissions, or eventual-consistency timing). For VPC-private ingress, the smoke test needs an in-boundary runner — plan it here.
...
After `tasks.md` is approved, the user assigns groups to parallel workers (agent
instances, developers, or a mix). For each task:
...
On resume:
1. Read `aidlc-state.md` for the active spec, phase, and active wave
...
### Execution Discipline (verification & environment reality)

These apply during task execution regardless of domain:

- **Preflight tool check + graceful degradation.** Before relying on a tool the tasks assume (linter, formatter, container runtime, CLI), verify it's present. If it's missing, substitute an equivalent gate (e.g. a formatter+validator in place of an absent linter) and record the substitution rather than silently skipping the check.
- **Failure/degraded paths must satisfy the same contract as the happy path.** A "never-empty handoff" or fallback output still has to pass the same schema/validation as the normal output. Prefer *omitting* unknown optional fields over emitting `null`; add an explicit test that validates the failure path.
- **Unit tests with fakes/mocks are necessary but not sufficient.** They cannot catch provider-side request validation, permissions-at-invoke, or environment/timing issues. When the spec's Definition of Done includes deployment, run a **live smoke test** (one real request through the deployed path) before declaring done.
- **Respect environment timing.** Some services are eventually-consistent for querying just-written data (e.g. log-query engines have ingestion latency; freshly-created resources may not be immediately queryable). Where a verification depends on such data, **poll until ready or budget a settling period** instead of asserting immediately.
- **Private/internal deployments can't be verified from a developer machine.** If ingress is VPC-private, plan an in-boundary runner (bastion/CI inside the network) or exercise the backend directly — decided at design time, not discovered at test time.
- **Capture self-fixes.** When you hit and fix an error mid-execution, log the problem, root cause, and fix (e.g. to an execution-lessons file in aidlc-docs) so it can be used as retrospective.
...
- **Parallelize by waves** — independent groups run concurrently; waves gate on dependencies
