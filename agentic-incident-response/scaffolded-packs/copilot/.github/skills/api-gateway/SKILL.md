...
12. **"Internal-only / not internet-facing" requires a private REST API — HTTP API (v2) has NO private endpoint mode.** An HTTP API with IAM (SigV4) auth is *authenticated* but its endpoint DNS is still publicly resolvable — authenticated ≠ off-the-internet. To keep traffic entirely on the AWS network, use a **REST API (v1) with `endpoint_type = PRIVATE`**, an **`execute-api` interface VPC endpoint**, and a **resource policy** that allows only the approved `aws:SourceVpce` (and denies otherwise). Decide this at design time from the requirement — retrofitting an API-type swap after build is costly. If a requirement mentions "internal", "private", "no public access", or "VPC-only", branch to private REST API immediately.
13. **A private API cannot be smoke-tested from outside the VPC** — its DNS only resolves through the interface VPC endpoint. Live end-to-end tests must run from an **in-VPC runner** (SSM/bastion host, in-VPC CI/CodeBuild) or over VPN/Direct Connect. Plan the test harness accordingly, or exercise the backend (e.g. the state machine/Lambda) directly for functional verification.
