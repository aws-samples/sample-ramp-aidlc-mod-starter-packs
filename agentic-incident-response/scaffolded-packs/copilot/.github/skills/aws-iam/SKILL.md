...
**Bedrock cross-Region / global inference profiles:**

- Invoking a **`global.*` (or cross-Region `<geo>.*`) inference profile** requires MORE than a single ARN grant. A naive policy that allows `bedrock:InvokeModel` on just the inference-profile ARN **silently fails at invoke time** because the profile routes to underlying foundation-model (FM) resources too. Grant **three resource statements** (confirm exact conditions against the [global cross-Region inference docs](https://docs.aws.amazon.com/bedrock/latest/userguide/global-cross-region-inference.html) — verify, don't rely on memory):
  1. the inference-profile ARN — `arn:aws:bedrock:<region>:<account>:inference-profile/<profileId>` (condition `aws:RequestedRegion = <region>`);
  2. the **in-Region FM** ARN — `arn:aws:bedrock:<region>::foundation-model/<modelId>` (note the empty account segment `::`; conditions `aws:RequestedRegion = <region>` + `bedrock:InferenceProfileArn` = the profile ARN);
  3. the **global FM** ARN — `arn:aws:bedrock:::foundation-model/<modelId>` (empty region AND account; condition `aws:RequestedRegion = unspecified` + `bedrock:InferenceProfileArn` = the profile ARN).
- The FM `<modelId>` is the profile id with the `global.`/`<geo>.` prefix stripped. Foundation-model ARNs are **not** account-scoped — including an account id causes authorization failures.
- Grant BOTH `bedrock:InvokeModel` **and** `bedrock:InvokeModelWithResponseStream` — the Bedrock Converse API uses streaming internally, so `InvokeModel` alone can still fail.
- A single-Region on-demand model (not via a cross-Region profile) needs only the plain FM ARN and does not require this three-statement pattern.
