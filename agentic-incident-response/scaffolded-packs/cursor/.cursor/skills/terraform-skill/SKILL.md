...
- **PowerShell mangles `=`-style flags.** Invoking terraform from PowerShell as `terraform plan -out=plan.bin` (or `-var=k=v`) can fail with `Error: Too many command line arguments` because the token is split. **Quote the whole flag**: `terraform plan "-out=plan.bin"`, `terraform apply "plan.bin"`, `terraform apply "-var=key=value"`. Not an issue in bash.
- **`archive_file` / asset bundling zips a directory as-is — it does not build.** If Lambda (or any) artifacts require a build step (installing/vendoring deps, staging source + assets into a package dir), that step must run **before** `plan`/`apply`, and again on code changes so the artifact hash updates. Terraform will happily zip an empty or stale directory and deploy a broken function otherwise.
- **Guard against applying to the wrong account/region.** Add a `check` block (1.5+) or a resource `precondition` that asserts `data.aws_caller_identity.current.account_id` (and region) equals the expected value, with an overridable variable. This surfaces a clear error at plan time instead of provisioning into the wrong account. (`check` blocks warn without blocking; a `precondition` on a resource hard-stops — pick per how strict the gate must be.)
