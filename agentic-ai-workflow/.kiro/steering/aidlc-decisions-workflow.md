...
**YOU MUST:**
1. Check if corresponding decision file exists in `.kiro/decisions/{spec-name}/`
2. If NO → Create decision file in `.kiro/decisions/{spec-name}/`, then STOP EXECUTION
3. If YES but empty → Ask user to fill it in, then STOP EXECUTION
4. If YES and completed → Proceed to create final document in `.kiro/specs/{spec-name}/`

**NEVER EVER:**
- Create requirements.md without completed `.kiro/decisions/{spec-name}/_decisions-requirements.md`
- Create design.md without completed `.kiro/decisions/{spec-name}/_decisions-design.md`
- Create tasks.md without completed `.kiro/decisions/{spec-name}/_decisions-tasks.md`
...
## Quick Reference
- Requirements phase → `.kiro/decisions/{spec-name}/_decisions-requirements.md` → `.kiro/specs/{spec-name}/requirements.md`
- Design phase → `.kiro/decisions/{spec-name}/_decisions-design.md` → `.kiro/specs/{spec-name}/design.md`
- Tasks phase → `.kiro/decisions/{spec-name}/_decisions-tasks.md` → `.kiro/specs/{spec-name}/tasks.md`
...
**🛑 ABSOLUTE RULE**: After creating ANY decision file in `.kiro/decisions/{spec-name}/`, you MUST STOP your execution and ask the user to review. DO NOT create the final document in `.kiro/specs/{spec-name}/` in the same turn.
...
**⚠️ ENFORCEMENT RULES**:
1. Before creating `.kiro/specs/{spec-name}/requirements.md` → Check if `.kiro/decisions/{spec-name}/_decisions-requirements.md` exists and is completed
2. Before creating `.kiro/specs/{spec-name}/design.md` → Check if `.kiro/decisions/{spec-name}/_decisions-design.md` exists and is completed
3. Before creating `.kiro/specs/{spec-name}/tasks.md` → Check if `.kiro/decisions/{spec-name}/_decisions-tasks.md` exists and is completed
4. If decision file doesn't exist → Create it in `.kiro/decisions/{spec-name}/`, STOP, and ask user to review
...
**Location**: `.kiro/decisions/{spec-name}/` directory (separate from spec outputs)
...
**Location**: `.kiro/decisions/{spec-name}/_decisions-requirements.md`
...
**Location**: `.kiro/decisions/{spec-name}/_decisions-design.md`
...
**Location**: `.kiro/decisions/{spec-name}/_decisions-tasks.md`

**🛑 CRITICAL FOR TASKS**: The tasks phase is often skipped. Before creating tasks.md, you MUST:
1. Check if `.kiro/decisions/{spec-name}/_decisions-tasks.md` exists
2. If not, create it in the decisions folder
3. STOP and ask user to review
4. Wait for user to provide input
5. Only then create `.kiro/specs/{spec-name}/tasks.md`
...
**🛑 STOP POINTS**:
- After creating `.kiro/decisions/{spec-name}/_decisions-requirements.md` → STOP, ask user to review
- After creating `.kiro/decisions/{spec-name}/_decisions-design.md` → STOP, ask user to review
- After creating `.kiro/decisions/{spec-name}/_decisions-tasks.md` → STOP, ask user to review
...
- Check correctness properties decision in `.kiro/decisions/{spec-name}/_decisions-design.md`
...
### File Management
**Lifecycle**:
- Decision files stored in `.kiro/decisions/{spec-name}/`
- Spec documents stored in `.kiro/specs/{spec-name}/`
