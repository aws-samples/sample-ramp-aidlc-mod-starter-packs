...
╚══════════════════════════════════════╝
     ...
     v
╔══════════════════════════════════════════════════════════╗
║  STAGE 3: Decomposition Plan — SINGLE KIRO SPEC         ║
║  .kiro/specs/decomposition-plan/                         ║
║                                                          ║
...
║  User picks which tasks to run in Kiro                   ║
╚══════════════════════════════════════════════════════════╝
     |
     v
  Complete
...
This workflow uses a **single Kiro spec** at `.kiro/specs/decomposition-plan/` that covers the entire modernization: planning, per-service extraction, and integration verification.
...
1. Generate `_decisions-requirements.md` with Kiro-recommended options
...
**Phase 2 — Design:**
1. Generate `_decisions-design.md` with Kiro-recommended options
2. Wait for user to fill in decisions
3. Read completed decisions → generate `design.md`
4. Wait for user approval of design

**Phase 3 — Tasks:**
1. Generate `_decisions-tasks.md` with Kiro-recommended options
2. Wait for user to fill in decisions
...
- [ ] 3. Decomposition Plan Spec — `.kiro/specs/decomposition-plan/`
...
**MANDATORY**: Load and follow all steps from `.kiro/steering/reverse-engineering.md`
...
**Single spec** at `.kiro/specs/decomposition-plan/` covering the entire modernization: decomposition decisions, target architecture, per-service extraction, and integration verification.
...
After user fills in decisions, generate the complete task list organized by section. **Users can pick which tasks to run in Kiro.**
...
After tasks.md is approved, the user selects which tasks to run in Kiro. For each task:
...
├── .kiro/
│   ├── specs/
│   │   └── decomposition-plan/            [THE single spec]
│   │       ├── _decisions-requirements.md
│   │       ├── requirements.md
│   │       ├── _decisions-design.md
│   │       ├── design.md
...
│   │       └── tasks.md
│   └── steering/
│       ├── aidlc-decisions-workflow.md
│       ├── reverse-engineering.md
│       └── skill-power-mcp-activation.md
│
├── aidlc-docs/
...
- **Single spec** — one Kiro spec covers the entire modernization lifecycle
- **Decision-driven** — every spec phase starts with a `_decisions-*.md` file; no document is generated without user decisions
- **Selective execution** — tasks.md is organized by section so users can pick which tasks to run in Kiro
